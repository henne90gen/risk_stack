// webgl.zig — app logic, runs identically on native (SDL3/OpenGL) and wasm.
//
// Compile to wasm:
//   zig build-lib src/webgl.zig -target wasm32-freestanding -dynamic \
//     -femit-bin=flip7.wasm
//
// Run natively:  the platform runner is wired in via `platform.run(App)` below.
...
var mvp_loc: i32 = undefined;
var color_loc: i32 = undefined;
...
const App = struct {
    pub fn onInit() void {
        ...
        gl.glGenBuffers(1, @as([*]u32, @ptrCast(&buf)));
    ...
    pub fn onResize(w: c_uint, h: c_uint, s: f32) void {
...
// Wire up the platform. On wasm this generates the four export symbols.
// On native, `main` calls platform.run(App) directly.
comptime {
...
// Native entry point — only compiled when target is not wasm.
pub fn main() void {
    platform.run(App);
}
