...
pub const card_data: [std.enums.values(Card).len][]const u8 = .{
    ...
    @embedFile("cards/back.png"),
};

test "card_data length matches Card enum" {
    try t.expectEqual(std.enums.values(Card).len, card_data.len);
}

test "card_data entries are non-empty PNG files" {
    const png_sig = "\x89PNG\r\n\x1a\n";
    for (card_data) |data| {
        try t.expect(data.len >= 8);
        try t.expectEqualSlices(u8, png_sig, data[0..8]);
    }
}
...
test "Card.fromPath round-trips every card" {
    for (std.enums.values(Card)) |card| {
        ...
        try t.expectEqual(card, Card.fromPath(path).?);
    }
}

test "Card.fromPath returns null for unknown paths" {
    try t.expect(Card.fromPath("/unknown.png") == null);
    try t.expect(Card.fromPath("/0.jpg") == null);
    try t.expect(Card.fromPath("") == null);
}
